...
for ticker in tickers:
    with open(f'data/{str(ticker[0])}.csv', 'r') as file:
        info = file.readlines()
    for i in range(1, len(info)):
        ...
        row[1] = to_convert_to_date_format[8:10] + '/' + to_convert_to_date_format[
                                                         5:7] + '/' + to_convert_to_date_format[:4]
        # print(row[1])
        row[8] = row[8].strip('\n')
        high_diff = 0 if i == 0 else float(row[7])
        my_dict = {"id": row[8], "date": row[1], "ticker": row[6],
                   "high": round(float(row[3]), 2), "low": round(float(row[4]), 2), "high_diff": high_diff}
        data.append(my_dict)
